...
/>`}
        ...
        {/* Custom Image Usage - Now using the customImage variable */}
        <ExampleContainer
          title="Custom ReactNode Image"
          description="Demonstrate using a custom ReactNode as an image with the Package icon."
          style={{ display: 'flex', justifyContent: 'center', padding: '24px' }}
          code={`const customImage = (
  <div style={{ 
    display: 'flex', 
    flexDirection: 'column', 
    alignItems: 'center', 
...
<Empty 
  image={customImage}
  description="Custom package icon"
>
  <Button>Add Package</Button>
        ...
        {/* Conditional Example - Using showCustomExample */}
        <ExampleContainer
          title="Conditional Rendering Example"
          description="Toggle between different empty states to show conditional rendering."
          style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}
          code={`const [showCustomExample, setShowCustomExample] = useState(false);
...
{showCustomExample ? (
  <Empty 
    image={<Package size={48} style={{ color: 'var(--empty-img-color)', opacity: 0.6 }} />}
    description="Custom state is active"
  >
    <Button onClick={handleToggle}>Switch to Default</Button>
  </Empty>
) : (
  <Empty description="Default state is active">
    <Button onClick={handleToggle}>Switch to Custom</Button>
  </Empty>
)}`}
        >
          <>
            <div style={{ textAlign: 'center', marginBottom: '16px' }}>
              <Button onClick={() => setShowCustomExample(!showCustomExample)}>
                Toggle State ({showCustomExample ? 'Custom' : 'Default'})
              </Button>
            </div>
            <Card style={{ padding: '24px', minHeight: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {showCustomExample ? (
                <Empty 
                  ...
                  description="Custom state is active"
                >
                  <Button onClick={() => setShowCustomExample(false)}>Switch to Default</Button>
                </Empty>
              ) : (
                <Empty description="Default state is active">
                  <Button onClick={() => setShowCustomExample(true)}>Switch to Custom</Button>
                </Empty>
              )}
            </Card>
          </>
        ...
        <ExampleContainer
          ...
          code={`// Custom image with working placeholder
<Empty
  image={
    <div style={{
      width: '80px',
  ...
  description="Custom empty state with styled placeholder"
>
        ...
        >
          <>
            <Card style={{ padding: '32px' }}>
              <Typography variant="h6" style={{ marginBottom: '16px', textAlign: 'center' }}>Custom Styled Placeholder</Typography>
              <Empty
                image={
                  <div style={{
                    width: '80px',
