...
export const Dropdown = forwardRef<HTMLDivElement, DropdownProps>(({
  ...
  const handleMenuItemClick = useCallback((info: { key: string; keyPath: string[]; item: MenuItem; domEvent: React.MouseEvent }) => {
...
export const DropdownButton = forwardRef<HTMLButtonElement, DropdownButtonProps>(({
  ...
  }, [onMenuClick, autoClose]);

  // Create enhanced menu items with click handlers
  const enhancedMenu = useMemo(() => {
    if (!menu) return undefined;
    
    const enhanceItems = (items: MenuItem[]): MenuItem[] => {
